...
public class SD4DevelopmentScopeManager{
	private static final Logger logger = LoggerFactory.getLogger(SD4DevelopmentScopeManager.class);
	private SD4DevelopmentGlobalScope globalScope = null;

	protected void setGlobalScope(SD4DevelopmentGlobalScope globalScope){
			this.globalScope = globalScope;
	}

 	public void initGlobalScope(MCPath modelPath) {
		SD4DevelopmentGlobalScope globalScope = (SD4DevelopmentGlobalScope) SD4DevelopmentMill.globalScope();
		globalScope.clear();
	 ...
	 */
	public void syncAccessGlobalScope(Consumer<SD4DevelopmentGlobalScope> action){
	...
	*/
	public SD4DevelopmentArtifactScopeWithFindings createArtifactScope(ASTSDArtifact ast, ISD4DevelopmentArtifactScope oldArtifactScope){
		Log.getFindings().clear();
		AtomicReference<ISD4DevelopmentArtifactScope> scopeRef = new AtomicReference<>(null);
		...
		syncAccessGlobalScope(gs -> {
			...
			try{
				scopeRef.set(SD4DevelopmentMill.scopesGenitorDelegator().createFromAST(ast));
			}catch(Exception e){
		...
		return new SD4DevelopmentArtifactScopeWithFindings(
			ast,
			scopeRef.get(),
			Log.getFindings()
			);
	...
	public Map<ASTSDArtifact, SD4DevelopmentArtifactScopeWithFindings> createAllArtifactScopes(Collection<ASTSDArtifact> astNodes){
		final Map<ASTSDArtifact, SD4DevelopmentArtifactScopeWithFindings> res = new HashMap<>();
		syncAccessGlobalScope(gs -> {
			clearGlobalScope();
			if(supportsIterativeScopeAppending()){
				...
				logger.warn("createAllArtifactScopes is not implemented. Please provide an implementation via the TOP-mechanism on SD4DevelopmentScopeManager");
			}
		});
		return res;
	}
